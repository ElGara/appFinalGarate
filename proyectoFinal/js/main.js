...
function creaNuevaDB(tx){
	...
	tx.executeSql("INSERT INTO agenda_curso (id,nombre,telefono,email,domicilio,categoria,nota,foto) VALUES (1,'Pizza Maria','+6699900970', 'pizzamaria@hotmail.com', 'Calle Marte #9614, col. San Pedro','Comida','Buena Pizza','')");
}
...
function queryDBUpdateForm(tx){
	...
	tx.executeSql('UPDATE agenda_curso SET nombre="'+$("#Newnombre").val()+'", telefono="'+$("#Newtelefono").val()+'",email="'+$("#Newmail").val()+'",domicilio="'+$("#Newdomicilio").val()+'",categoria="'+$("#Newcategoria").select()+'",note="'+$("#NewNota").val()+'"foto = "'+$.imageURL+'" WHERE id='+$.id);
}
...
function queryDBInsertForm(tx){
	//var cat = $("#categoria").find("input:checked").val();
    var select = document.getElementById('categoria');
    var cate = select.value;
    $.imageURL = "../img/defectoImagen.png"
	tx.executeSql("INSERT INTO agenda_curso (id,nombre,telefono,email,domicilio,categoria,nota,foto) VALUES ('"+$("#nombre").val()+"','"+$("#telefono").val()+"','"+$("#mail").val()+"','"+$("domicilio")+"','"+cate+"','"+$("Nota")+"','"+$.imageURL+"')", [], newFormSuccess, errorDB);
}
//Home
